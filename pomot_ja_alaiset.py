...
def laske_alaiset(tyontekija: Tyontekija): 
    omat_alaiset = len(tyontekija.alaiset)
    
    if tyontekija.alaiset == 0:
        return 0 
    
    else: 
        alaisen_alaiset = len([laske_alaiset(tyontekija) for tyontekija in tyontekija.alaiset])
        return omat_alaiset + alaisen_alaiset   
